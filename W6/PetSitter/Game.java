/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package petsitter;

import java.util.ArrayList;
import java.util.Scanner;

public class Game {

    //private Dog pet;
    private ArrayList<Pet> pet = new ArrayList<Pet>();

    public void playGame() {

        Scanner scan;
        int pilih = -1;
        do {
            System.out.println("CHOOSE :");
            System.out.println("1. New Pet");
            System.out.println("2. Activity with Available Pet");
            System.out.println("Choose :");
            scan = new Scanner(System.in);
            int a = scan.nextInt();
            if (a == 1) {
                Pet apet;
                System.out.println("Choose Pet Type: 1. Dog  2. Smart Dog   3. cat");
                scan = new Scanner(System.in);
                int type = scan.nextInt();
                if (type == 1) {
                    apet = new Dog();
                } else if(type == 2){
                    apet = new SmartDog();
                }else{
                    apet = new Cat();
                }

                System.out.println("Pet's name : ");
                String newname = scan.next();
                apet.setName(newname);
                //memasukkan objek Dog ke dalam arraylist
                pet.add(apet);
            } else {
                Pet adog = null; //variable adog untuk menampung objek anjing yg namanya sesuai dengan yg dicari

                while (adog == null) {
                    System.out.println("Which pet's name ?");
                    String name = scan.next();

                    //mencari onjek dog yang punya nama seperti yang ditulus user
                    for (int i = 0; i < pet.size(); i++) {
                        if (pet.get(i).getName().equalsIgnoreCase(name)) {
                            adog = pet.get(i); //meemegangkan objek pet dalam arraylist yg namanya sesuai ke variable adog
                            break;
                        }
                    }
                }
                System.out.println("What you want to do");
                System.out.println("1. Play with Pet");
                System.out.println("2. Feed the oet");
                System.out.println("3. give the pet a bath");
                System.out.println("4. tell it to sleep");

                if (adog instanceof SmartDog) {
                    System.out.println("5. Learning with the it");
                }

                System.out.println("0. Exit");
                scan = new Scanner(System.in);
                System.out.print("Pilih : ");
                pilih = scan.nextInt();
                switch (pilih) {
                    case 1:

                        if (adog != null) { //variable adog akan tetap null (kosong) jika tidak ada objek dog dalam arraylist yg namanya sesuai 
                            adog.play();
                            // menampilkan status dari anjing..
                            System.out.println(adog.getName()
                                    + "'s status : Happiness (" + adog.getHappiness()
                                    + ") |  Cleaniness (" + adog.getCleaniness()
                                    + ") | Fullness (" + adog.getFullness()
                                    + ") | Health (" + adog.getHealth() + ")");
                        } else {
                            System.out.println("Pet not found");
                        }

                        break;
                    case 2:

                        if (adog != null) { //variable adog akan tetap null (kosong) jika tidak ada objek dog dalam arraylist yg namanya sesuai 
                            adog.eat();
                            // menampilkan status dari anjing..
                            System.out.println(adog.getName()
                                    + "'s status : Happiness (" + adog.getHappiness()
                                    + ") |  Cleaniness (" + adog.getCleaniness()
                                    + ") | Fullness (" + adog.getFullness()
                                    + ") | Health (" + adog.getHealth() + ")");
                        } else {
                            System.out.println("Pet not found");
                        }

                        break;
                    case 3:

                        if (adog != null) { //variable adog akan tetap null (kosong) jika tidak ada objek dog dalam arraylist yg namanya sesuai 
                            adog.bath();
                            // menampilkan status dari anjing..
                            System.out.println(adog.getName()
                                    + "'s status : Happiness (" + adog.getHappiness()
                                    + ") |  Cleaniness (" + adog.getCleaniness()
                                    + ") | Fullness (" + adog.getFullness()
                                    + ") | Health (" + adog.getHealth() + ")");
                        } else {
                            System.out.println("Pet not found");
                        }

                        break;
                    case 4:

                        if (adog != null) { //variable adog akan tetap null (kosong) jika tidak ada objek dog dalam arraylist yg namanya sesuai 
                            adog.sleep();
                            // menampilkan status dari anjing..
                            System.out.println(adog.getName()
                                    + "'s status : Happiness (" + adog.getHappiness()
                                    + ") |  Cleaniness (" + adog.getCleaniness()
                                    + ") | Fullness (" + adog.getFullness()
                                    + ") | Health (" + adog.getHealth() + ")");

                        } else {
                            System.out.println("Pet not found");
                        }

                        break;
                    case 5:

                        if (adog instanceof SmartDog) {
                            SmartDog smart = (SmartDog) adog;
                            smart.learn();
                            System.out.println(adog.getName()
                                    + "'s status : Happiness (" + smart.getHappiness()
                                    + ") |  Cleaniness (" + smart.getCleaniness()
                                    + ") | Fullness (" + smart.getFullness()
                                    + ") | Health (" + smart.getHealth() 
                                    + ") | Intelligence (" + smart.getIntelligence() 
                                    + ")");

                        }
                }
                System.out.println("");
                System.out.println("");

            }

        } while (pilih != 0);
    }
}
